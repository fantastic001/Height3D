...
class MyFilledSurfaceModel : public FilledSurfaceModel
{
public:

	MyFilledSurfaceModel(int precision) : FilledSurfaceModel(precision) 
	{
...
class MyFilledSurfaceTestLoop : public SDLLoop 
{
...
public:
	MyFilledSurfaceTestLoop(Window *w) : SDLLoop(w) 
	{
...
protected:
	void onInit() 
	{
		...
		cube = scene.addObject(new SceneObject(
			new MyFilledSurfaceModel(200), // use surface parametrized model to draw this object
...
BOOST_AUTO_TEST_CASE( functional_filled_surface )
{

	cout << "Running phong";
	Window win(512, 512, "tutorial");
	MyFilledSurfaceTestLoop loop(&win);
	loop.run();
}
