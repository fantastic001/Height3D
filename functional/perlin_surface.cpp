...
class MyPerlinSurfaceTestLoop : public SDLLoop 
{
...
public:
	MyPerlinSurfaceTestLoop(Window *w) : SDLLoop(w) 
	{
...
BOOST_AUTO_TEST_CASE( functional_perlin_surface )
{

	cout << "Running phong";
	Window win(512, 512, "tutorial");
	MyPerlinSurfaceTestLoop loop(&win);
	loop.run();
}
